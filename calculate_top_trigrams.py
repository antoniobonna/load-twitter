...
file = 'trigram.csv'
query_app = "SELECT distinct company_id FROM twitter_dw.company WHERE company != 'SOFISA'"
query_company = 'SELECT distinct company FROM twitter_dw.vw_twitter'
query_data = "SELECT DISTINCT month,year FROM twitter_dw.vw_twitter WHERE company_id = '{}' AND month != date_part('month',current_date) AND year != date_part('year',current_date) ORDER BY 2,1"
query_comentario = "SELECT tweet FROM twitter.tweets_all WHERE company_id = '{}' AND date_part('year',datetime) = {} AND date_part('month',datetime) = {} AND not tweet is NULL"
tablename = 'twitter_dw.trigrams'
